...
base_num_airtankers = [1] #list of the number of airtankers at each base
base_num_bird_dogs = [0] #list of the number of bird dogs at each base
base_airtankers_cruising = [[3]] #Cruising speed of each airtanker, km/min
base_airtankers_fight = [[2]] #Fight fire flying speed of each airtanker km/min
base_airtankers_circling = [[2]] #circling speed of each airtanker km/min
base_airtankers_max_time = [[500]] #Max mins airtankers fly w/o return to a base
base_airtankers_max_distance = [[500]] #max distance without return to a base km
...
def update_points(fires, points):
    ...
    for x in range(len(points)):
        ...
        for y in range(len(fires)):
            if distance(fires[y].longitude, fires[y].latitude, \
...
def select_airtanker(fire, bases):
    ...
    for x in range (len(bases)): #Finds closest airtanker
        for y in range(len(bases[x].airtankers)):
            try:
                if distance(bases[x].airtankers[y].longitude, \
                            bases[x].airtankers[y].latitude, \
                            fire.longitude, fire.latitude) < distance(\
                                bases[base_num].airtankers\
                                [airtanker_num].longitude, \
                                bases[base_num].airtankers\
                                [airtanker_num].latitude, \
...
def airtanker_arrival(env, fire, airtanker):
    ...
    travel_dist = distance(airtanker.longitude, airtanker.latitude, \
                               fire.longitude, fire.latitude)
...
def dispatch_airtanker(env, fire, bases):
...
def print_simulation_results(stats, points):
    ...
    for x in range(len(points)):
        ...
        print "Percent of days burned: %0.3f" \
              %(sum(points[x].burned) / len(points[x].burned))
