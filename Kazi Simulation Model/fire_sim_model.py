...
class User_input(object):
    ...
    def __init__(self, inputs):
        ...
        #self.time_until_start = inputs[46] #~ NEED TO ADD, minutes
        ...
        try:
            ...
            while temp != '' and temp != '\n':
                ...
                inputs[30] = int(temp2[3+int(temp2[2])])
...
class Fire(object):
    ...
    def max_growth(self, inputs):
        ...
        self.time = ((self.time_at_ignition // LENGTH_DAY) * LENGTH_DAY +
...
class Airtanker(object):
    ...
    def suitable(self, fire, time):
        ...
        in_range = (obj_distance(self, fire)-EPS) <= self.initial_attack_radius
        #Determine if past the airtankers start service time
        past_start = (time + EPS) >= self.start_time
        #Determine if before the airtankers end of service time
        before_end = (time - EPS) <= self.end_time
        ...
        if self.after_fire_protocol == 3:
            ...
            #if this makes it fly to long, it instead returns home
...
def update_statistics(stats, fires, bases, points, inputs): #add base stats
    ...
    for x in xrange(len(fires)): #Increase each value by the amount for a fire
        ...
        days_passed = fires[x].time_at_ignition // LENGTH_DAY
        cur_fire_ig_time = fires[x].time_at_ignition - days_passed * LENGTH_DAY
        ...
        if x == 0:
            ...
            if days_passed == fires[x-1].time_at_ignition // LENGTH_DAY:
...
def select_airtanker(env, fire, bases):
...
def return_airtanker_process(env, bases, travel_time, airtanker,
    ...
    If 0 for return to home base
    If 1 to return to closest base
    If 2 to stay at the fire
    If 3 return to closest base, but return to home base at end of day'''
    travel_distance = travel_time * airtanker.cruising_airspeed
    prot = airtanker.after_fire_protocol
    #Currently both if in queue and not give identical actions but can be change
...
def dispatch_airtanker(env, fire, bases, fires, inputs):
    ...
    base_num, airtanker_num = select_airtanker(env, fire, bases)
    time_at_req = env.now
                        ...
                        for y in xrange(len(bases[z].airtankers)):
                            if bases[z].airtankers[y].start_time + EPS >=\
                               new_time:
                                ...
                                reqs.append(env.timeout(new_time - env.now))
