...
def lightning_is_detected(env, inputs):
    ...
    if random.random() < PROB_DETECTED_LIGHTNING: #80% Chance fire is detected
        detected = True
    else:
        detected = False
    return detected
...
def human_is_detected(env, inputs):
    ...
    if random.random() < PROB_DETECTED_HUMAN: #80% Chance fire is detected
        detected = True
    else:
        detected = False
    return detected
...
def lightning_detect_time(env, inputs):
    ...
    detect_time = random.expovariate(1.0 / DETECT_TIME_LIGHTNING)# 2 hours
    return detect_time

def human_detect_time(env, inputs):
    '''Return human caused fire detection_time'''
    detect_time = random.expovariate(1.0 / DETECT_TIME_HUMAN)# 2 hours detect
    return detect_time
...
def create_airtankers(base_num, env, inputs):
    ...
    for x in xrange (num):
        (cruising, fight, circling, lat, lon, fly_after_dark, drop_type,
                size, fuel_cap, fuel_con, after_fire_prot,
         initial_attack_radius) = get_airtanker_stats(
...
def lightning_wait_coef(env, inputs):
    '''Determines the coefficient for waittime based on time of day'''
    return 1.0
...
def get_time_per_drop(lake_distance, airtanker_fight_airspeed):
    '''Return the time per drop'''
    return 2.0 * lake_distance / airtanker_fight_airspeed + DROP_TIME
...
def at_control_fire_time_calc(env, fire, airtanker, inputs):
    ...
    time_per_drop = get_time_per_drop(lake_distance, airtanker.fight_airspeed)
    ...
    for x in xrange (len(bases)): #Finds closest airtanker
        for y in xrange(len(bases[x].airtankers)):
            if bases[x].airtankers_resource[y].count == 1 and base_num == -2.0\
                ...
                try:
                    new_dist = obj_distance(bases[x].airtankers[y], fire)
                except IndexError:
                    continue
                if new_dist < bases[x].airtankers[y].initial_attack_radius:
                    base_num = x
                    airtanker_num = y
                continue
            try:
                new_dist = obj_distance(bases[x].airtankers[y], fire)
                if new_dist < obj_distance(
                    bases[base_num].airtankers[airtanker_num],fire) and \
                    new_dist < bases[x].airtankers[y].initial_attack_radius:
                    base_num = x
                    airtanker_num = y
            except IndexError:
                continue
        ...
        for x in range(len(bases)):
            ...
            for y in range(len(bases[x].airtankers)):
                ...
                if new_dist < bases[x].airtankers[y].initial_attack_radius:
                    ...
                    temp = env.timeout(inputs.length_run + 1)
